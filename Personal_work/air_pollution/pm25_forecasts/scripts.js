...
async function displayStatistics(stationData, pm25Data, timeStep) {
    ...
    Object.keys(areaStats).forEach(area => {
        ...
        const maxStationElement = document.getElementById(`max-station-${area}`);
        ...
        if (values.length > 0) {
            ...
            maxStationElement.textContent = maxStation.name;
        } else {
            [avgElement, stdElement, maxElement, maxStationElement].forEach(el => el.textContent = 'N/A');
        }
    });
}
